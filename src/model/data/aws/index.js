...
async function deleteFragment(ownerId, id) {
  try {
    ...
    if (!fragment) {
      // Fragment doesn't exist, so nothing to delete
      logger.warn({ ownerId, id }, 'Fragment does not exist, nothing to delete');
      return;
    }
    ...
    try {
      // Second, delete the data from S3
      const params = {
        Bucket: process.env.AWS_S3_BUCKET_NAME || 'fragments',
        Key: `${ownerId}/${id}`,
      };
      ...
      // Log the error but don't throw - we already deleted the metadata
      // which is the primary record of the fragment's existence
      logger.warn({ err, ownerId, id }, 'Error deleting fragment data from S3 - continuing anyway');
    }
  } catch (err) {
    // If there's an error checking if the fragment exists or deleting metadata
